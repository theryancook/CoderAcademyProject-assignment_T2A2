...
####  d. Describe the way data is structured within the app
For a 2 Way Marketplace such as Gumtree, the main relationship is between the users and the products. I’d structure the data into at least 2 separate tables - a product table, and a user table. As an example, let’s say you have User A who’s just listed a product on the marketplace to sell. The product has a description, an image, a price, and importantly, the listing itself belongs to the User A. User A’s name is displayed, showing that they own this product, as well as other user information such as contact details & location. If the user has many products, all these different products will have a relationship with the same one user. Products will also have a relationship to a second user, that being the user who purchases the item, and that information may include name, contact details etc

####  e. Identify entities which must be tracked by the app
The different entities that need to be tracked can be broadly separated out into 2 different categories. Firstly, we’d have the entities that are associated with the products listed on the marketplace. The types of information here would include but aren’t limited to an image of the product, the location, price, condition, description, date listed & the user who listed it. The other category is information about the marketplace users (both buyers and sellers) and would include username, physical address, phone number, ip address & language. By storing this type of information, users are then able to see a comprehensive library of product listings, and both sellers and buyers are able to contact and negotiate the sale of the item.

####  f. Identify the relationships and associations between the entities you have identified in part (e)
* Users have one or many Products
* Products have one buyer (User)
